...
sys.path.append(src_dir + "/external/iree-llvm-sandbox/python/")
sys.path.append(os.path.join(os.environ["MMPERF_BUILD_DIR"], "mlir/tools/iree_llvm_sandbox/python_package"))
os.environ["MLIR_RUNNER_UTILS_LIB"] = os.path.join(os.environ["MMPERF_BUILD_DIR"], "mlir/lib/libmlir_runner_utils.so")
os.environ["MLIR_C_RUNNER_UTILS_LIB"] = os.path.join(os.environ["MMPERF_BUILD_DIR"], "mlir/lib/libmlir_c_runner_utils.so")
...
# CHECK-NOT: FAILURE
def main(argv):
  # 1.Initialize/Find 2 Best Expert.
  # 2.Start tuning the param of best expert.
